...
from ..models import IocFeed, RuleState

logger = logging.getLogger("douglas.jobs")
...
def job_dict(j: Job) -> dict:
    return {
        ...
        "dispatched_at": _iso(j.dispatched_at),
...
class LiveFinding(BaseModel):
    """One finding streamed while the sweep is still running."""
...
class ProgressRequest(BaseModel):
    ...
    # Findings produced since the last report. A sweep can run for eight
    # minutes, and a critical finding sitting on the host for seven of them
    # because the bundle has not uploaded yet is the wrong shape for an
    # incident. These land in the console as they are found.
...
# without limit.
MAX_ACTIVITY = 200
# Per post, not per hunt. Stops a runaway rule turning one progress tick into
# a multi-megabyte insert.
MAX_LIVE_FINDINGS = 200
...
def report_progress(
    ...
    # Findings streamed mid-sweep. Stored as they arrive so the console fills
    # in while the hunt runs rather than staying empty for eight minutes and
    # then producing everything at once.
    live_rows: list[dict] = []
    if payload.findings:
        disabled_rules = {
            r.rule_id for r in db.query(RuleState)
            .filter(RuleState.enabled == False).all()  # noqa: E712
        }
        # Which positions this hunt already holds. A progress post that timed
        # out and was retried resends the same findings, and they must not
        # land twice.
        known = {
            s for (s,) in db.query(Finding.seq)
            .filter(Finding.job_id == job.id, Finding.seq.isnot(None)).all()
        }
        for item in payload.findings[:MAX_LIVE_FINDINGS]:
            if item.seq <= 0 or item.seq in known:
                continue
            rule_id = (item.rule_id or "")[:32]
            ...
            if sev not in ("CRITICAL", "HIGH", "MEDIUM", "LOW", "INFO"):
                sev = "INFO"
            row = Finding(
                job_id=job.id, agent_id=agent.id, hostname=agent.hostname,
                rule_id=rule_id, severity=sev,
                title=(item.title or "")[:512], evidence=item.evidence or "",
                mitre=(item.mitre or "")[:32], why=item.why or "",
                artifact=(item.artifact or "")[:128],
                occurred_at=(item.occurred_at or "")[:64],
                seq=item.seq,
            )
            db.add(row)
            known.add(item.seq)
            live_rows.append({
                "seq": item.seq, "rule_id": rule_id, "severity": sev,
                "title": row.title, "evidence": row.evidence[:400],
                "mitre": row.mitre, "artifact": row.artifact,
                "hostname": agent.hostname, "job_id": job.id,
            })

    try:
        db.commit()
    except IntegrityError:
        # Two posts racing on the same positions. The database settled it; the
        # progress itself is not worth failing the request over.
        db.rollback()
        live_rows = []
    ...
    if live_rows:
        # Separate message so a view that only cares about findings does not
        # have to unpack every progress tick to find out nothing arrived.
        broadcast({"type": "finding.live", "job_id": job.id,
                   "hostname": agent.hostname, "findings": live_rows})

    return {"ok": True, "accepted": len(live_rows)}
...
async def upload_results(
    ...
    # except the findings that already arrived on the live stream. Those carry
    # a position, the bundle contains the same positions, and deleting them
    # here only to insert identical rows a moment later would make every
    # finding on screen blink out and back during the upload. Anything a
    # genuine re-upload changes is still reconciled: a position present in the
    # bundle is skipped, and one that is not is left as the record of what the
    # sweep reported at the time.
    db.query(Finding).filter(Finding.job_id == job.id,
                             Finding.seq.is_(None)).delete()
    db.query(TimelineEvent).filter(TimelineEvent.job_id == job.id).delete()

    counts = {"CRITICAL": 0, "HIGH": 0, "MEDIUM": 0, "LOW": 0, "INFO": 0}

    # Rules an operator switched off. The collector is told about these at the
    # start of a hunt and skips them, but the check is repeated here so an
    # older agent — or one that started before the change — cannot reintroduce
    # findings from a rule the console says is off. Two layers, because the
    # cost of the second is one set lookup per row.
    disabled_rules = {
        r.rule_id for r in db.query(RuleState).filter(RuleState.enabled == False).all()  # noqa: E712
    }
    dropped_disabled = 0

    # Positions already stored from the live stream. The bundle is the whole
    # sweep, including everything sent while it ran, so without this every
    # streamed finding would appear a second time the moment the hunt ended.
    already = {
        s for (s,) in db.query(Finding.seq)
        .filter(Finding.job_id == job.id, Finding.seq.isnot(None)).all()
    }
    streamed = len(already)

    for index, row in enumerate(finding_rows, start=1):
        if not isinstance(row, dict):
            continue
        if index in already:
            # Counted in the totals below, because it is a finding of this
            # hunt — it simply arrived earlier.
            sev_seen = str(row.get("Severity") or row.get("severity") or "INFO").upper()
            if sev_seen in counts:
                counts[sev_seen] += 1
            continue
    ...
    if dropped_disabled:
        logger.info(
            "Hunt %s: %d finding(s) discarded because their rule is switched off",
            job.id, dropped_disabled)
    ...
    # Score the external addresses this hunt just found, in the background.
    # By the time an operator opens the graph the triage order is already
    # right, without the upload having waited on a third party — and with no
    # providers configured this does nothing at all rather than failing.
    try:
        from ..routers.enrichment import active_keys
        from ..services import enrichment as enrich_svc

        if active_keys(db):
            graph_obj = job.graph if isinstance(job.graph, dict) else {}
            addresses = [
                (ep.get("address") or "").strip()
                for ep in (graph_obj.get("endpoints") or [])
            ]
            addresses = [a for a in addresses if enrich_svc.is_enrichable(a)]
            if addresses:
                from ..routers.enrichment import enrich_async
                enrich_async(addresses)
    except Exception:  # noqa: BLE001 - enrichment is a bonus, never a blocker
        pass
    ...
    return {
        ...
        # Findings a switched-off rule discarded on the way in. Counted and
        # returned rather than dropped quietly: a hunt that looks unusually
        # clean should be traceable to the rules somebody turned off, not to
        # an unexplained gap.
        "dropped_disabled": dropped_disabled,
        # How many of these the console already had from the live stream, so
        # the agent can tell the difference between "nothing new" and "nothing
        # found".
